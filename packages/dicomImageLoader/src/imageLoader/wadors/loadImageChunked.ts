...
import {DICOMLoaderIImage, DICOMLoaderImageOptions, ImageFrame} from '../../types';
import getPixelDataFramesChunked from './getPixelDataChunked';
...
function loadImageChunked(
  imageId: string,
  options: CornerstoneWadoRsLoaderOptions = {}
): { promise: Promise<DICOMLoaderIImage[]>; cancelFn: undefined } {
  ...
  const promise = new Promise<DICOMLoaderIImage[]>((resolve, reject) => {
    ...
    function sendXHR(imageURI: string, imageId: string, mediaType: string) {
      ...
      return getPixelDataFramesChunked(imageURI, imageId, mediaType)
        .then((resultFrames) => {
          const transferSyntax = getTransferSyntaxForContentType(
            resultFrames.contentType
          );

          const images: DICOMLoaderIImage[] = [];

          for (const image of resultFrames.frames) {
            createImage(
              image.id,
              image.pixelData,
              transferSyntax,
...
export default loadImageChunked;
